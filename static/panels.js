...
const APP_TITLEBAR_KEYS = {
  ...
  profiles: 'tab_profiles', todos: 'tab_todos', context: 'tab_context', insights: 'tab_insights', logs: 'tab_logs', settings: 'tab_settings',
};
...
async function switchPanel(name, opts = {}) {
  ...
  if (nextPanel === 'context') loadContextBrief();
...
// ────────────────────────────────────────────────────────────────────────────
// Context brief panel — session goal, requests, accomplished, remaining work.
// Two layers served by POST /api/session/context-brief: a deterministic
// assembly (always available, instant) and a persisted auxiliary-LLM
// narrative regenerated by a background job (POST .../context-brief/refresh,
// polled via GET .../context-brief/status). Read-only display state: the
// brief is never injected into the agent context.
// ────────────────────────────────────────────────────────────────────────────
...
function _appearancePayloadFromUi(){
  ...
  return {
    ...
    chat_activity_display_mode: chatActivityModeSel&&(chatActivityModeSel.value==='transparent_stream'||chatActivityModeSel.value==='hide_all_activity'||chatActivityModeSel.value==='transparent_live_compact_settled')
...
function _syncChatActivityDisplayModeControl(mode){
  const next=mode==='transparent_stream'||mode==='transparent_live_compact_settled'||mode==='hide_all_activity' ? mode : 'compact_worklog';
...
function _syncTransparentEventTimestampsControl(enabled, mode){
  ...
  const activeMode=mode==='transparent_stream'||mode==='transparent_live_compact_settled'||mode==='hide_all_activity' ? mode : (window._chatActivityDisplayMode||'compact_worklog');
  const timestampsCapable=activeMode==='transparent_stream'||activeMode==='transparent_live_compact_settled';
  const checkbox=$('settingsTransparentEventTimestamps');
  if(checkbox){
    checkbox.checked=next;
    checkbox.disabled=!timestampsCapable;
    checkbox.style.opacity=timestampsCapable?'':'0.5';
  }
...
function _applyShowBackgroundWakeupsSetting(value,opts={}){
  const enabled=value!==false;
  const changed=(window._showBackgroundWakeups!==false)!==enabled;
  window._showBackgroundWakeups=enabled;
  const control=$('settingsShowBackgroundWakeups');
...
async function _autosaveAppearanceSettings(payload){
  try{
    ...
    if(saved){
      _applyShowBackgroundWakeupsSetting(saved.show_background_wakeups);
...
async function loadSettingsPanel(){
  try{
    ...
    if(typeof _applySessionNavigationPrefs==='function') _applySessionNavigationPrefs();
    _applyShowBackgroundWakeupsSetting(settings.show_background_wakeups);
    ...
    if(showBackgroundWakeupsCb){
      showBackgroundWakeupsCb.onchange=function(){
        _applyShowBackgroundWakeupsSetting(this.checked);
        _scheduleAppearanceAutosave();
      };
    }
...
function _applySavedSettingsUi(saved, body, opts){
  ...
  _applyShowBackgroundWakeupsSetting(body.show_background_wakeups,{rerender:false});
...
async function saveSettings(andClose){
  ...
  body.chat_activity_display_mode=((($('settingsChatActivityDisplayMode')||{}).value==='transparent_stream')
    ...
    ||(($('settingsChatActivityDisplayMode')||{}).value==='transparent_live_compact_settled'))
