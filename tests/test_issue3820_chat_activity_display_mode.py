...
def test_transparent_event_row_quiet_metadata_visual_rhythm():
    ...
    transparent_remove_block = remove_block[remove_block.index("if(isTransparentLiveMode())"):remove_block.index("const turn=$('liveAssistantTurn');")]
    ...
    transparent_finalize_block = finalize_block[finalize_block.index("if(isTransparentLiveMode())"):finalize_block.index("if(!isSimplifiedToolCalling())")]
