...
def test_frontend_reload_width_paths_read_the_live_ceiling():
    ...
    the clamp-vs-full-transcript decision has a single owner (it now clamps to
    the ceiling instead of dropping the window, and only falls back to the full
    transcript when clamping would shrink the visible transcript). It still
    reads the live `_msgLimitMax`, which is what this test guards.
    """
