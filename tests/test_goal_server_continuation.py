...
def _replace_record(gc, session_id, record):
    with gc._registry_transaction() as registry:
        registry["intents"][session_id] = record
        gc._save_locked()
...
def test_schedule_after_compression_atomically_completes_parent_intent(continuation_store):
    ...
    assert settled_parent["completed_at"] == 20.0
...
def test_schedule_after_compression_rejects_wrong_parent_stream(continuation_store):
    gc, _ = continuation_store
    parent = _schedule(gc, sid="session-parent", stream="judge-parent")
    parent.update(
        status="running",
...
def test_schedule_after_compression_rejects_foreign_live_child(continuation_store):
    ...
    _replace_record(gc, "session-child", child)
