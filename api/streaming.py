...
def _active_turn_boundary_is_authoritative(identity):
    return (
        _active_turn_boundary_is_valid(identity)
        and identity.get('agent_turn_boundary_resolved') is True
    )
...
def _find_active_turn_checkpoint_index(
    ...
    *,
    ...
    ``allow_exact_prefix`` preserves the proven legacy exact-prefix projection,
    while shifted repeated-prompt candidates still fail closed.
    """
    result_messages = list(result_messages or [])
    if not isinstance(identity, dict):
        return None
    if _active_turn_has_checkpoint(result_messages, identity):
        for idx, message in enumerate(result_messages):
            if _active_turn_token_matches(message, identity):
                return idx
    expected_text = identity.get('text') if identity.get('text') is not None else msg_text
    strict_boundary = _active_turn_boundary_is_authoritative(identity)
    legacy_boundary = (
        type(identity.get('current_turn_user_idx')) is int
        and identity['current_turn_user_idx'] >= 0
        and type(identity.get('turn_id')) is str
        and bool(identity['turn_id'].strip())
    )
    if require_authoritative and not strict_boundary:
        return None
    if not strict_boundary and not legacy_boundary:
        previous_context = list(previous_context or [])
            ...
            with _agent_lock:
                ...
                with _stream_writeback_stage(_writeback_timings, "merge_result"):
                    ...
                    if cancel_event.is_set():
                        ...
                        put('cancel', _cancel_payload_for_stream(stream_id))
                        return
                    _defer_result_settlement_for_auth = False
                    if isinstance(result, dict) and result.get('error') is not None:
                        _pre_settlement_classification = _classify_provider_error(
                            '',
                            result.get('error'),
                            silent_failure=False,
                            result=result,
                        )
                        _defer_result_settlement_for_auth = (
                            _pre_settlement_classification['type'] == 'auth_mismatch'
                        )
                    if not _defer_result_settlement_for_auth:
