        ...
        for attempt in range(2):
            ...
            if messages_changed or context_changed:
                try:
                    ...
                    logger.debug(
                        "Failed to persist collapsed duplicate assistant rows for %s",
                        sid,
                        exc_info=True,
                    )
                    repaired = False
            ...
            # For a legacy sidecar, cache authoritative metadata facts only
            # after the exact sidecar revision has been verified stable.
    ...
    try:
        with closing(open_state_db_readonly(db_path)) as conn:
            ...
            if before_int is not None:
                if before_int <= 0:
                    return _state_db_session_messages_result(
                        [], None, with_revision=with_revision
                    )
            ...
            if (
                ...
                and limit is None
                # ``before`` is a bounded display cursor (LIMIT/OFFSET), so the
                # fetched rows are not the full active segment the revision
                # fence describes. Fail closed rather than stamp a partial read.
                and before is None
